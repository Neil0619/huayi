...
批量队列只接收原生文件窗口选择的路径；网页端不能提交路径或命令。选用批量结果只能传本次
队列随机 ID，经会话令牌、准确 Host/Origin 与空 JSON 请求体验证后，再按原流程校验缓存。
任务串行，停止只跳过剩余文件。播放器换片请求校验准确官网来源、已成功导入的播放器窗口和
该次 nonce；旧窗口、错误 nonce 或第三方页面不能触发原生选片。批量不新增扩展权限或外部请求。

快速导入不再要求目录选择。Node 仅向已由原生文件窗口选定并校验的缓存签发 256 位随机
只读流地址，限制为 GET/HEAD、单 Range、准确 Host 与允许的 Origin；不接受任意路径。
每次打开校验路径和文件身份，最多四个能力、16 个活动响应，进程结束全部撤销。
官网收到单个视频能力和选中字幕，不收到管理令牌、目录句柄或打开器身份密钥。
MAIN world 只临时映射这次导入的精确 File 对象，消费后/超时/离页恢复原生 URL API。
后台与内容脚本仅认可严格回环流格式，视频匿名 CORS 保持捕获可用。Chrome 本机服务访问
权限仍由浏览器正常管理；无新增 Manifest 权限，不通过浏览器禁用安全标志实现导入。
能力 URL 属于敏感本机数据，不进入诊断、Provider 或 Git；官网作为媒体接收者已能读取该视频。
...
快捷方式读写使用 Windows `IShellLinkW` / `IPersistFile` Unicode 接口，仍先核对已存在入口的
目标和完整参数，仅接受本工具的新旧安装路径。PowerShell 通过 UTF-16 编码命令接收固定程序，
用户路径独立由环境参数传入，配置文件按 UTF-8 读取；不拼接用户路径为脚本代码。
