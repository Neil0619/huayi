...
且有可见布局框的英文字幕，不调用字幕 API、OCR、timed-text 或整段 transcript。为恢复
YouTube 跨时间切分的当前句，Content Script 只在页面内存保留最近 20 秒、最多 16 次有效观察
且总计不超过 2,000 字符的有界字幕状态；不持久化、不记录日志，也不读取 URL、标题、视频 ID、
频道或播放时间。seek、广告、直播、播放器替换、视频导航、离开 `/watch` 和 Content Script
销毁都会立即清空该状态。普通非 YouTube 网页不创建字幕观察器。YouTube DOM 选择器变化、
直播、Shorts、广告、非英文字幕或超长字幕都失败关闭，不影响普通网页选区。

点击“译”只发送无页面数据 warmup；冻结字幕仍留在页面本地。只有用户随后点击解释或翻译时，
当前选区和组装后的目标句才进入既有分析通道。缓存中的其他观察内容不会展示或发送。单词查词
只发送单词；显式加入欧路时沿用同一目标句作为英文语境。该路径不发送 URL、标题、视频 ID、
频道、播放时间、字幕历史或相邻句，也不新增 Manifest 权限。
