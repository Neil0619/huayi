...
Windows 不读取 macOS 钥匙串，也不接收命令参数、环境变量、仓库文件或扩展消息中的 Key。
配置命令使用固定 PowerShell helper，通过隐藏的 `Read-Host -AsSecureString` 构造
`PSCredential`，再用 `Export-Clixml` 写入
`%LOCALAPPDATA%\Huayi\native-host\deepseek-credential.xml`。Windows 会用 DPAPI 保护密码
字段，因此只有同一台机器上的同一 Windows 用户能够解密。

Host 每次请求以固定 PowerShell 路径、固定 helper 和参数数组导入该凭据；输出限制为 8 KiB、
超时 5 秒，Key 仅短暂存在于子进程 stdout、Host 内存和 DeepSeek Authorization Header。
stdout Native Messaging、stderr、错误、测试和快照不得包含 Key。DPAPI 保护静态存储，但不能
防御以同一 Windows 用户权限运行的恶意进程。

Windows 只访问官方 DeepSeek HTTPS endpoint，不启动 Codex，不读取 `~/.codex`，也不构造
欧路、OpenAI 或 Compatible Provider。Chrome 注册仅写入精确 HKCU Native Messaging 键，
卸载只删除该键和带 Huayi ownership marker 的目录。
...
Windows 安装不探测 Codex 或 macOS Keychain，只校验 SEA `.exe`、PowerShell helper、模型
Schema 和扩展 ID。安装器写入 `%LOCALAPPDATA%\Huayi\native-host` 与精确 HKCU Chrome 键；
重复安装保留 DPAPI 凭据，卸载只删除带合法 ownership marker 的目录和该注册表键。
