...
JSON、欧路授权、Codex 认证、token 或环境数据；stdout 只写 Native Messaging 帧。
...
`~/Library/Application Support/Huayi/native-host/`、Chrome Native Messaging 清单路径和欧路
钥匙串 service `com.huayi.codex_bridge.eudic` / account `authorization` 不变。完整卸载才先
删除精确钥匙串项，再删除经过所有权验证的 Huayi 文件；它不会删除 Chrome 父目录、其他
Native Messaging 清单或其他凭据。
