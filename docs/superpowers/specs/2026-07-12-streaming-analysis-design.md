...
| 分析阶段                             | 查询状态                          | 右上角展示               |
| ------------------------------------ | --------------------------------- | ------------------------ |
| 任意适用阶段（含 loading/streaming） | `checking` / `absent` / `unknown` | 可点击的“加入欧路生词本” |
| 任意适用阶段                         | `present`                         | 不可点击的“已加入生词本” |
| 任意适用阶段                         | `saving`                          | 不可点击的“正在添加…”    |
| 任意适用阶段                         | 添加成功或确认已存在              | 不可点击的“已加入生词本” |

按钮从分析开始就固定在不随正文滚动的词卡顶部；后续查询返回 `present` 时在原位置替换为
“已加入生词本”，不隐藏、不产生明显布局跳动。被动查询失败只变为 `unknown`，不遮挡分析
结果，也不提前显示错误；用户显式点击添加时仍按现有规则展示未配置、授权、限流或网络错误。
...
Escape 会取消当前视图仍在运行的分析与查询，并把已接受的添加请求从视图脱离；添加继续在
后台运行，其终态只负责结束请求生命周期，不得重开或改写当前或后续浮层。标签页销毁和请求
超时仍会向包括添加在内的关联请求发送定向 `cancel`。
...
- “生词”与“已加入生词本”在流式期间出现，以及查询结果在固定顶部位置原位替换。
- 外部点击、新选区、关闭和 Escape 取消分析/查询但不取消已接受的添加；标签页销毁、超时和
  App Server 崩溃能取消正确请求。
