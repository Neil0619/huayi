...
状态包含字幕代次、源／译轨 readiness、固定双语、按住 Shift+Z 或字幕右上角“中”角标的临时展示、活动选区和暂停所有权：

- 点击 CC 旁“中”切换当前视频固定双语；不带 Ctrl／Alt／Meta 的非重复 `Shift+Z` keydown 临时
  显示中文。字幕卡右上角
  的“中”角标也可按住显示中文，松开、指针取消或丢失捕获即恢复。键盘和角标独立持有，任一仍按住
  即持续显示。
- keyup、窗口 blur、页面隐藏、导航或销毁清除临时展示；可编辑控件聚焦、有选区或有结果卡时不
  消费 `Shift+Z`。实际中文条件为译轨 ready 且固定双语或任一临时展示来源仍按住。
...
- 有效非空选区冻结字幕句。仅视频原本播放时暂停，并以播放器和字幕代次记录暂停所有权；原本
  暂停时关闭不得播放。
- 有选区或卡片时，第一次点击播放器视频空白区由 Huayi 消费；同一指针手势随后的原生 `click`
  也必须被消费，避免 Huayi 恢复播放后又被播放器切回暂停。该手势关闭卡片、清除选区，并只恢复
  Huayi 暂停的视频；原本暂停的视频保持暂停。控件、字幕和 Huayi UI 不属于空白区。
- 用户主动播放、seek、视频结束、导航或播放器替换立即撤销暂停所有权。显式加入生词本 pending
  时外点可收起词卡；已接受的写入在后台继续，不取消或重开。
