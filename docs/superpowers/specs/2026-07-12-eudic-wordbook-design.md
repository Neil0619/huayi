...
保留结果区滚动位置和合理焦点。按钮固定在词卡顶部、紧邻词头且不随正文滚动；模型结果尚未
完整生成时也可根据状态查询结果显示“生词”或“已加入”。处于“正在添加”时，外部点击、关闭
按钮、Escape 或创建新选区只收起当前词卡，不取消已经接受的写入；后台终态仍须被消费，但不
得重开或改写后续词卡。标签页销毁、Content Script 卸载或请求超时仍可取消执行中的写入。
...
新选区、关闭和 Escape 只定向取消当前视图的分析与状态查询。已接受的 `add-word` 与视图生命
周期解耦并在后台继续；超时和标签页销毁仍可取消它，迟到终态不能重开或改写浮层。
