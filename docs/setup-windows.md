...
提前处理多集可点「批量预处理视频」并在文件窗口按 Ctrl / Shift 多选。队列串行处理，成功后
点「选用此视频」开始原有学习流程；「停止剩余任务」保留当前和已完成缓存。播放器里的
「打开另一个视频」会直接请求选片，不必先手工寻找打开器标签页。更新后退出旧打开器再启动，
并重载原固定目录的语见扩展及播放器页，避免新旧通信代码混用。

新版媒体缓存位于每个原视频目录下的「缓存视频」，文件名保留原片名与集数。
准备好后直接「开始学习」，无需复制路径或授权目录。已有结果可点「打开缓存视频」选择对应
`.浏览器.mp4`，自动匹配缓存字幕；选择原片同样先验证复用，缺失或被修改的缓存才重新准备。
Chrome 首次可能提示允许官网访问本机服务；拒绝时按播放器提示调整该网站权限并重试。
视频从本机按需读取，不等待整部复制进浏览器。原配置 `cacheRoot` 仅查找可复用的旧缓存。
安装目录中的 `browser-origin.json` 保持本机服务身份，不要分享或提交它。
