...
`openai-responses` 或 `openai-compatible-http`。活动请求期间修改配置只影响下一个请求；任一
Provider 失败都不会自动迁移或回退。Codex warmup 继续完成 App Server 初始化；两个 HTTP
Provider 的 warmup 只验证本地配置读取能力，不读 Key、不发 HTTP。该严格 Host 控制边界不向
...
未知内容不会被认领或覆盖。v0.6.0 使用 wire v4 并拒绝 v3，因此 Extension 和 Host 必须使用
...
`com.huayi.codex_bridge.compatible_http` / `api-key` 及有效的 Provider/Compatible 配置不变。
...
`fetch`；受控端点、拒绝重定向、限制响应体且不自动重试，因此没有新增生产依赖。
