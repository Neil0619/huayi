...
候选文档更新：2026-09-21；公开页面以正式部署后的版本为准。
...
- 连接安全：为登录、注册与设备配对等接口防止滥用，服务端处理连接 IP，保存结合服务器秘密计算的
  限流标识、操作类型、时间窗口和计数；不使用 GPS，也不据此生成广告画像；
- 本机数据与秘密：BYOK、欧路凭据及插件本机词库保存在 Store Extension。BYOK/欧路凭据不发送给
  语见服务器；本机词条只有在用户开启以后新词复制，或显式确认批量导入时，才向 Web 创建独立副本。

插件在本机处理当前网址、域名及 YouTube 视频标识，用于逐站规则和字幕等页面功能，不读取 Chrome
浏览历史列表，也不自动上传完整网页或完整浏览历史。模型请求只包含所选内容及必要语境；学习内容的
来源标题由用户填写或受信的固定来源类型提供。语见不读取 Google Drive 等额外账号资料。

## 扇贝回填共享

主动开启回填并确认账号共享后，语见接收欧路和本机词头、来源类别、回填目标、必要设置和成功、
跳过或结果待确认进度，并检查当前账号云端生词。首次检查包含已同意来源的已有词；欧路默认在
本地时间 08:00 检查，浏览器恢复后补查；账号云端词头每 15 分钟刷新。该过程不上传欧路或本机词的
解释、原句、URL、标题或模型结果，不因此创建云端学习库词条，也不调用模型。

未关联时账本保存在本机；共享进度按账号隔离，各设备须使用同一扇贝账号。语见不读取扇贝完整
词表，也不能验证扇贝账号身份。停用回填保留已有进度；删除学习词条不清除成功与跳过历史。
完整账号导出 format 5 包含已共享的回填记录；删除账号清除账本。断开、卸载或本机清空不删除
已经产生的云端与第三方副本，欧路和扇贝中的副本须通过相应服务删除。
...
- Supabase/Vercel 承载身份、数据库、API 与 Web；当前不启用 Google 登录；Resend 用于验证、
  密码恢复和安全通知；
- 欧路在用户同意后用于手动导入/导出及主动开启的每日回填检查；导出接收词头和可选原句。扇贝
  每批最多预填 100 个目标词头，最终添加始终由用户亲自点击；notes、语境释义、标题与来源不发送。
  欧路凭据由插件直达欧路。
...
- 连接安全的限流记录尚未设置自动删除期限；计数窗口不代表记录保留期限。这类记录使用哈希标识，
  删除账号不保证同步清除。托管服务的访问日志按其服务设置与隐私规则处理，不适用上述错误诊断的
  30 天期限；
