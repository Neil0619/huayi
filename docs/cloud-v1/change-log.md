...
## 2026-09-24：asbplayer 保留局部原字幕并继续学习

- 真实素材抽样发现，少量片头说明、中文行中的英文名称和符号会让旧的单轨双语确认拒绝整份字幕。
  现在仅在存在独立英文／中文行证据后接管；含中文的行保留英文名称，无法确定独立英文的片段完整
  恢复原字幕，后续明确对白自动恢复学习。仍不从混排的同一行猜测英文，不丢弃字幕内容。
- 上游完整快照没有文本时显示输入检查提示，不再显示空轨道选择器；重新载入有效字幕后可正常确认。
- 音视频编解码与图片字幕仍由播放器和输入准备决定，本次没有加入转码、OCR、网络字幕下载或新权限。
  具体边界见 [本地视频学习](asbplayer-local-video.md)。

## 2026-09-23：Store 接入官方 asbplayer 本地视频学习

- 官方网页负责播放和文本字幕解析，语见接管确认轨道后的画面学习字幕；OCR、转码、整片翻译、
  Anki、侧栏和时间点回跳不属于首版。
- 新增仅限官方域和播放帧的 MAIN/ISOLATED 入口；被动监听内部频道，双侧净化、限流、有界内存和
  生命周期失效；只在完整字幕、偏移、主视频、轨道均验证后以可逆样式隐藏原字幕。
- Settings v7 增加独立模式／快捷键并迁移 v1–v6；Store 消息 v6 增加专用设置和字幕句边界，旧页刷新。
  本机 source=web、云 web-selection 以及 Classic wire v7 保持既有契约。
- 设计、安全、使用与验收边界统一见 [asbplayer 本地视频学习](asbplayer-local-video.md)。
...
## 2026-09-07：邮箱注册先验证，再设置并确认密码

- 用户选择“邮箱 → 验证码验证成功 → 设置并确认密码 → 完成”；验证码直接在 Web 输入，自动沿用
  已填邮箱，取消打开邮件确认表单和重复输入邮箱的必经步骤。
- 注册进度由独立浏览器证明与服务端加密状态持有；只有密码设置成功后才完成邀请并进入工作台。
  同一浏览器可在有界窗口恢复，重发、验证与设置密码使用原子状态推进，旧邮件流程继续兼容。
- 详见 ADR-0026、`email-first-password-signup.md` 及同步更新的 product/API/security/data-model/testing。

## 2026-09-04：插件与学习工作台采用紧凑布局和渐进帮助

- Popup 固定 340px，调色板统一控制 Store 四套外观，设置失败恢复原主题；联网指示灯仅表示许可，
  不探测网络。待上传只在有内容或异常时展示，账号连接放在紧凑行与设置顶栏；
- 网站规则独立管理，沿用既有 `hostname/includeSubdomains/action`，具体域名和同域精确规则优先，
  根域关闭允许单站开启；手动输入经 URL 和离线公共后缀校验，不迁移设置；
- Web 合并顶栏，普通标题 28px、内容最大 1280px、设置二级导航 208px 左侧栏。普通帮助按需展开，
  费用、上传与删除确认继续显式展示；固定外部词典跳转不接受任意 URL；
- 视觉权威同步至 `seen-said-ui-design-system.md`；验证与交付边界见 `ui-refinement-acceptance.md`。
...
- Vercel Sensitive Environment Variable 不可解密回读，不能再要求用户手工复制、记忆或比较
  `CRON_SECRET`。首次环境不能再要求“先完成密码恢复才 provision”，因为恢复邮件 worker 自身依赖该
  bearer；受控 provision 现在只接受“唯一可 claim 的密码恢复且 R3-C 为空”或“唯一可 claim 的 R3-C”
  两种精确状态，并要求 Cron 精确 absent；
- 首次路径固定为“提交一次恢复 → provision → exact-SHA API release → recovery worker
...
- provision 创建或复用 `huayi_api_origin`/`huayi_cron_secret`，并把同一 bearer 在进程内 upsert 到 API
  ...
  release state，并只在 upsert 成功后原子写入带随机 `releaseAttemptId` 且
  `provenance=cron-bootstrap-provision` 的 schema-v3
  `candidate-recorded`；
- 环境 upsert 后必须由同一 state 部署同一 clean exact SHA；API/Web deployment metadata 必须精确绑定该
  ...
  只接受 bootstrap-provenance schema-v3 `complete` 并重新通过 API/Web runtime attestation，再从 Vault
  读取同一值；普通 release 与 legacy schema-v1/v2 state 仅保留 status 兼容，
...
## 2026-09-04：密码恢复同密码反馈修正

- Hosted 实测确认恢复完成请求已通过 Cookie、Origin、CSRF 和短时 session 校验，Supabase 因新密码与
  当前密码相同而拒绝更新；后续短时间重试又按 30 秒完成 lease 失败关闭，密码与恢复状态均未误报成功。
- Web 在输入前明确新密码不能与当前密码相同，统一失败提示也要求改用不同密码并稍后重试；不把
  `same_password` 映射成公开专用错误，继续丢弃 Provider 原始 message/status，避免密码相等性探测信号。
- Provider 与 Web 回归分别锁定统一安全错误和可操作提示；真实完成仍需新恢复邮件与不同的新密码，不能
  用本地或日志证据替代 Hosted 改密、旧 session 撤销、安全通知和重新登录验收。
