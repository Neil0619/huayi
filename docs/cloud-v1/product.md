...
- 新用户必须持有未绑定邮箱、72 小时过期且只能领取一次的邀请链接；领取后可选择 Google 或
  邮箱密码，邮箱密码必须验证邮箱。
- 邮箱注册按“填写邮箱 → 站内输入六位验证码 → 设置并确认密码 → 完成”推进。邮箱只填写一次，
  验证成功前不显示密码；两次密码不一致时留在当前页。刷新 `/join` 可在原浏览器恢复进度，重发验证码
  沿用已绑定邮箱。只有密码设置与邀请建档都完成才进入工作台，详见 `email-first-password-signup.md`。
...
- “今日练习”默认目标为 10 个学习项，用户可修改；服务端按当前时钟和北京时间（Asia/Shanghai）先选择到期项，再以
...
- Web `/settings/account` 从服务器读取当前账号的 UTC 月度限额、已结算、活跃预留、剩余、使用百分比
  ...
  读取和更新 1–100 的每日练习目标，以及三项账号级扩展偏好；每日练习统一按北京时间换日，
  不提供时区设置，旧账号保存的其他时区不影响结果。新设置只影响后续查询、
