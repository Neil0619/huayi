...
- 插件平台查询结构修复回归模拟“已显示语境义、常见义和短语，但完整输出含无效字段”的流式响应，
  验证准确字段反馈和输出约束使既有第二次调用恢复成功；该样本为合成回归，不声称恢复了原始模型 JSON。
  同时覆盖 JSON 语法错误、嵌套字段缺失、语言/音标约束、段落类型错配、空数组与数量/长度边界、
  一次修复上限、两次调用计费保留和全部八种 action/selectionKind 组合。
- 校验诊断覆盖 initial/repair 两阶段、最多 8 条原因、未知键名及错误值不泄露、生成编号过滤、诊断
  写入失败或修改参数不影响修复。公开查询错误继续只返回原有稳定错误码，不携带内部字段或 Provider 原文。
...
2026-09-02 新增的 `acceptance-hosted-cron-bootstrap*.mjs` 把此前人工 secret continuity 门收敛为
主控制流、共享失败关闭支持、R3-C/recovery 状态判定、只读 recovery SQL renderer 和 Vercel adapter。
默认测试只使用 fake Keychain、psql、Cron status、runtime snapshot 与 HTTP，必须覆盖：
...
- plan 零 I/O；provision 先验证 clean repository candidate、专用 Cron
  ...
  source；Vercel upsert 成功后才原子写带注入式随机 `releaseAttemptId` 且
  `provenance=cron-bootstrap-provision` 的 schema-v3 `candidate-recorded`，
...
- release 的 Vercel create 必须设置 `forceNew=1`；正常路径不得在 create 前 find，只有 create 响应丢失
  ...
  都只接受 fixed branch 上同一 clean/pushed/disarmed SHA 的 bootstrap-provenance schema-v3 state 已由新
  API/Web deployment 推进到 `complete`，并在读取 Vault 前重新通过 runtime attestation；普通 release 与
  schema-v1/v2 complete 仍可只读 status，但 bootstrap delivery 必须拒绝；之后只从
...
首次真实验收必须另行按固定顺序运行 provision → exact-SHA API release → recovery → 用户完成改密 →
R3-C deliver → 用户通知收件确认 → Cron status/apply；离线 GREEN 不能提前关闭其中任何一步。已有唯一
claimable R3-C 的恢复路径可跳过 recovery/改密段，但不能跳过 exact-SHA deployment 或最终收件门。
...
- 发布材料测试必须精确锁定 `apps/web/vercel.mjs` 实际生成的全 path header rule；缺 CSP、
...
- CSP 必须锁定 `default-src 'self'`、`base-uri/object/frame-ancestors/frame-src/worker-src` 的拒绝边界，
  ...
  script/connect。正式 profile 只允许 `api.seen-said.cn` 和独立项目 `pxqqgxfumovegbcxnmzb`，不得
  包含验收域或验收数据库；跨环境、未知 profile 和实际候选配置读取必须有离线回归；
- 旧静态 CSP 缺失的回归继续保留精确断言；新增配置测试执行候选的实际 `vercel.mjs`，不能只解析旧 JSON。
  离线通过不代表线上生效；
...
- 两张 private 表必须 forced RLS，PUBLIC、Supabase API roles、business/context-setter/runtime 与专用
  `NOLOGIN` executor role 均无表直权，trigger functions 对这些角色也无 execute；
