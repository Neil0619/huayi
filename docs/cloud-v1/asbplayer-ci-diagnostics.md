...
后续入口已切到 Classic 短语拖选诊断，用于调查候选 `2897154` 在 Windows CI 的一次工具栏缺失。
原字体／快捷键诊断仍可按下文命令执行，其准确旧提交为 `78e68bd`。新工作流每个平台最多 100 轮
原真实拖选及解释流程，保留原 5 秒工具栏期望和 10 秒结果期望，第一次失败即停止并非零退出。
没有派发合成选区或替换鼠标操作。事件监听只记录固定合成页面的有界事件名、选区长度／是否匹配、
范围数、指针坐标、焦点和布局，不记录选中文本、网页内容或请求载荷；网络只允许本机离线服务。
新工作流仅上传 `selection.jsonl`，不上传截图或 trace。未复现不能证明原失败已修复。
诊断已通过 lint 和 100 项测试发现检查。额外严格 TypeScript 检查未通过：原
`journey-helpers.ts` 的 `dispatchFixtureSelection` 在 75／76／79 行缺少 undefined 收窄。
该函数不在本诊断的真实拖选路径中；原 helper 与产品候选相同，未为取得类型检查通过而改写它。

```powershell
pnpm exec playwright test apps/extension/e2e/selection-diagnostic.spec.ts --max-failures=1
```
...
原字体／快捷键工作流只上传两个 `.codex-pet-runs/targeted-ci/` 下的 JSON 回执，不上传媒体、字幕、频道、页面 URL、
