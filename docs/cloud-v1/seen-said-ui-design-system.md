...
2026-09-04 根据截图反馈收紧工作台与插件布局；本文件下述紧凑排版、统一插件换肤和帮助规则取代
此前的大标题、分离导航与 Popup 仅切换材质的设计。具体验收见 `ui-refinement-acceptance.md`。
...
- 普通工作页主标题：28px，行高 1.35；标题、筛选和主要操作集中在内容顶部，不使用海报式大标题；
- 区块标题：1.25–1.55rem，字重 600；
- 正文：14–15px，行高 1.55–1.7；帮助正文不小于 14px；
- 普通工作页移除装饰性英文 eyebrow 和重复说明，不能用缩小文字解决信息过量；
...
所有主题复用同一结构和组件，不得按主题改变布局。品牌、主导航和外观入口合并为紧凑顶栏，
内容最大宽度 1280px，顶栏至内容间距 24px。普通页面不为装饰或静态说明单独占一张卡片。

- 桌面顶栏最小高 64px；1440×900 下，包括 42px 验收横幅时，首个业务区应出现在顶部 240px 内；
- 待整理只保留一个标题，“待分析／待收藏”和显示状态组成同一工具栏；
- 设置使用 208px 左侧二级导航，保留原账号、设备、数据路由和权限；
- Popup 的 html/body 均固定 340px，品牌、开关与快捷操作不压缩到竖排；账号为一行，空队列隐藏；
- 插件设置的“网站管理”独立成页，列表每页 20 条，常用设置只保留总开关和管理入口；
- 插件设置的“模型与密钥”集中展示联网授权、服务商选择和对应密钥；切换服务商只切换密钥表单，
  保留各自未保存的输入，不自动保存、删除或回显已有密钥。划词后的默认动作继续放在常用设置；
- 搜索输入和按钮统一高度、底部对齐；不存在下一页时不展示无效翻页操作。

## 6. 响应式规则

验收视口固定为 1440、1024、768、390px；实现断点如下：

- `>1120px`：完整紧凑顶栏和页面所需的列表／详情结构；
- `≤1120px`：压缩品牌副标题和外观入口宽度，仍保留明确文字；
- `≤768px`：主导航和设置二级导航折叠；设置内容单列，不与侧栏横向争抢空间；
- 窄屏普通标题仍为 28px，内容两侧 16px，长列表和操作组按可用宽度换行；
...
- 选择即时生效；Web 保存失败时继续本页预览并提示未保存；Store 保存失败时恢复原主题并提示；
...
返回触发器。初始数据载入完成时不得把焦点自动移到静态区块标题，避免把 focus ring 误呈现为内容
边框。永久删除、撤销和管理员危险操作必须保留现有二次确认与权限门。

### 7.5 帮助与说明

普通用法收敛到标题或字段旁的小问号，支持悬停、聚焦、点击固定和 Esc 关闭。上传范围、第三方
费用、不可逆删除后果必须在相应确认操作时明确展示，不能只藏进 hover；实现术语、内部状态名和
重复加载成功信息从普通用户界面移除。密钥仍只显示已配置状态和圆点占位，不回显原值。
...
- Popup 调色板与 Options 使用同一 Store 外观存储；保存后同步到其他插件页面及已打开词卡；
- Popup 不再用调色板切换 `pearl | parchment`；既有材质值保持兼容且不迁移；
