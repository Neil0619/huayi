...
2. Windows 可通过[本机打开器](asbplayer-local-opener.md)选择原 MKV，自动准备音轨和文字字幕再进入官网。
   结果保存在原视频旁的「缓存视频」目录；准备后直接开始学习，已处理视频可用「打开缓存视频」。
   无需再选择目录，Chrome 首次可能要求允许官网访问本机服务，视频通过本机 Range 按需读取。
   也可在 Chrome 打开 [官方网页](https://app.asbplayer.dev/)，通过网页入口拖入可播放的视频和文本字幕。
   不需要 asbplayer 扩展、Native Host 或 Anki。
3. 学习层会列出轨道编号和文本预览。根据内容确认英语轨道、中文轨道；单轨分行双语可选择同一轨道，
   中文行中可保留英文名称。根据完整字幕内容预选明确的单轨双语或唯一中英文轨道，仍需点击确认；
   歧义保留人工选择。没有中文则选择「无中文轨道」。不会把上一集的轨道编号套用到下一集。
...
媒体播放结束后，未被选区冻结的学习字幕会清空，即使最后一句字幕的时间超出视频长度；
跳回视频有效时间后恢复对应字幕，无需重新确认轨道。
...
- 英文轨道中的纯音乐符号等无字母片段保留完整原字幕，其余英文继续学习；
  不因一条 `[♪♪♪]` 拒绝整份文件。整轨没有可识别英文或英语轨道混入中文时仍拒绝确认。
- 单轨双语至少有一段清楚、独立的英文行和含中文的行；中文行可包含 Wi-Fi 等英文名称。
  片头说明、仅中文、符号或无法识别的行不会再使整份字幕失效：这些片段完整恢复播放器原字幕，
  后续明确的英文对白自动恢复学习，无需重新确认。发生重叠时也优先保留原显示，不删原文。
  不从同一行中猜测、截取英文；整份都只有含糊混排时仍需另备独立文本轨道。
- 上游未读到任何字幕文本时显示输入检查提示，不提供空轨道选择。文件须可被浏览器读取；
  可用 UTF-8 或较短路径的副本作对照，不覆盖原文件。测试工具在 Windows 应使用支持长路径的参数。
- 容器扩展名不能证明浏览器支持其中的音视频编码。先用官方网页确认画面和音频都正常；不兼容输入
  可使用独立本机打开器复制 H.264/HEVC 视频、将不兼容音轨转为 AAC。扩展自身不执行转码。
- 本机打开器自动提取内嵌文字字幕；图片字幕仍需另行 OCR 并校对；压制在画面中的字幕不能直接被划词。
...
官网 blob 媒体或严格限定的回环流地址，以及 channel 经核对后才能订阅。
回环流仅为本机打开器的 `http://127.0.0.1:<非特权端口>/stream/<64hex>`，不接受任意 HTTP 地址。主视频按来源、播放器结构和可见布局唯一确认，排除同源
缩略图视频。实际播放操作经媒体会话接口执行，并以真实媒体事件撤销过期暂停归属。
关闭词卡前还会核对暂停位置和原生跳转状态；用户跳转与关闭发生在同一个事件任务时，
即使 `seeking` 尚未派发也不能自动恢复播放。
...
`asbplayer-file-compatibility.spec.ts` 另外覆盖真实 Store 中的片头／混排片段原字幕回退、
后续英文自动恢复、中文行保留英文名称、学习与收藏，以及空字幕提示和重新载入恢复。
这些用例使用合成文本，不把用户影视或字幕放进仓库。

两脚本可显式指定 `--browser-executable <绝对路径>` 和 `--deny-local-fonts`，默认不改变测试浏览器
或权限。后者只拒绝当前隔离配置中官网的字体权限，回执记录实际权限读数；真实浏览器回归验证该
设置在辅助函数返回后仍有效，且不影响其他 origin 或独立 context。基础脚本另可使用
`--common-popup-window` 在真实 Windows 缩放校验下调整实际弹窗至 1000×700，记录调整前后尺寸
及 DPR。命令与限制见 Git 接续说明；附加条件下的成功不能覆盖官网默认全屏或自动弹窗失败。
