...
的 YouTube 英文 CC 和快捷键正常；隔离浏览器的真实站点完整学习流程仍未验证。最新产品构建的
100% 实际 Store、真实 BFCache 和官网基础／扩展矩阵已补验通过，仍不能声明全部
...
这次恢复由用户确认，没有新增代理实读 DPR。既有 100%／150% 实测回执保持原身份与条件。
...
- 用户已确认日常 Chrome 的真实 YouTube 英文 CC 与快捷键正常；完整学习、首次加载、SPA 和字幕
  切换仍未验证。后续需在隔离 Chrome 中取得可用字幕，再加载同一候选、以合成 Provider 补测划词、
  关闭恢复及切换；不能读取或复制日常浏览器凭据来解除隔离环境阻塞。
