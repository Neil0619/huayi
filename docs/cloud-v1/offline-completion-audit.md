...
Store release 和 production dependency audit。完整 staged manifest 为
...
8 commits、111 files、`+3007/-1175`；范围审查未发现协议包、wire/schema、依赖锁、Windows 系统原语、
SEA/安装器或 Native Messaging 传输变化，也未发现 added-line secret、生成物或可执行产物。最终候选
再增加本节所在的 docs-only 冻结提交，因此相对上次 Windows 代码共 9 commits。
