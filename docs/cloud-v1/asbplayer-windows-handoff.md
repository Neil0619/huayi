...
2026-09-25 最新缓存直开与免目录选择候选为
`50826f57718f79cb9118b87032769a4a73802bd5`，分支仍为
`codex/asbplayer-windows-validation-fixes`。新增「打开缓存视频」；选择原片时自动校验并复用
有效缓存。准备好后直接开始学习，以本机 Range 流取代目录授权和整片预读。
Chrome 首次可能要求官网访问本机服务的网站权限，拒绝时可重试；没有扩大 Manifest 权限。
准确候选的 macOS/Windows CI 均通过，各 253 项浏览器检查通过。固定安装、150% 两集
官网完整学习、耗时和未验证项见[缓存直开与免目录选择回执](asbplayer-cache-stream-validation.md)。
新版不沿用旧版目录授权测试作为证据；100% 和本轮原生交互的边界以新回执为准。
...
该历史轮完整 CI 当时尚未通过：首轮 Windows 原生快捷方式测试失败，补齐真实旧安装夹具及失败诊断后，
新候选双平台 CI 尚在运行。后续全面检查已继续定位并修复，当前状态以顶部最新回执为准。
...
[Windows 原视频打开器](asbplayer-local-opener.md)。该轮新界面的 100%、原生文件选择和桌面快捷方式交互尚待实机验证。
后续已观察到桌面双击打开 Chrome 和前置选片窗口；完整原生选片仍未在该轮完成，新界面 100% 按用户决定暂缓。
不能沿用下方旧候选的 100% 通过结果。
