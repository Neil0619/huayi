...
- 结果卡不显示 × 关闭按钮；Esc、卡外点击只结束显示订阅；停止是独立按钮，并等待服务端确认。相同输入重开会加入原
  请求或读取完成缓存；模式分别计入身份，外观变化不计入。
- 易混词、同义词辨析每条占满卡片宽度，词性保持完整，说明在词条下方换行；正式与备用样式均使用
  单列条目，只有短语配对和字段详情显式启用两列，避免嵌套列把英文和中文说明挤碎。
...
`practice_session_items.rated_at` 在首次自评时写入且保持不变，今日进度统一按北京时间统计；一次自评仍
...
停止确认或连接中断后清除“正在停止”的临时提示，保留已校验的部分内容；在同一卡片重试时，
新的生成重新启用停止按钮。完成结果先于停止确认到达时，继续显示已保存的完整结果。

扩展将流式 `delta.text` 作为原始片段按序拼接，接受独立到达的空格、换行和制表符；
仍拒绝空字符串及超过 4096 字符的片段，完整结果继续要求非空正文。不能把完整字段的
`trim()` 非空校验复用于流式片段，否则会在合法空格处提前显示失败，而服务端仍正常完成。
...
提示版本为 `web-deep-analysis-v2.5`。请求保留 `reasoning_effort: low`，与已有非思考请求及
本机验收契约兼容。短语 4096、句子/段落 8192 token 上限、最多两次调用的费用预留、
90 秒期限、完整结构校验与教学内容要求保持不变。结束诊断仅用固定标签区分空正文、
预算耗尽与缺少结束数据，不记录具体模型内容。

完整正文仍未通过校验时，分别诊断 JSON、输出结构、分析单元数量及最终业务约束；区分首次
生成与结构修复。日志仅保留固定阶段、已知字段路径和错误类别，隐藏未知字段名及所有值，
并限制嵌套分支遍历和日志条数。诊断写入失败不影响修复、计费或公开错误。

真实复验确认首次与修复结果均可能因候选排序号不连续而被拒绝。模型候选通过私有结构校验后，
服务器按数组展示顺序生成连续排序号，再执行最终业务校验。此步骤不重排候选，不更改内容、
候选 ID 或所属分析单元；重复 ID、未知引用、错误单元归属及无效字段仍被拒绝。
