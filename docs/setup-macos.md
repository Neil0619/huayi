...
`verify:macos` 会运行离线指令检查、格式、Lint、类型、单测、Chrome Playwright、构建和 diff
...
Chrome 每天本地时间 08:00 检查欧路；若该时刻 Chrome 或设备不可用，则恢复后尽快补扫，下一次
仍固定在次日 08:00。点击角标会打开扇贝并预填最多 100 个词，最终“批量添加”
