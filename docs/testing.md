...
macOS CI 的整个作业预算为 60 分钟：曾在第 233/250 条浏览器检查通过后触及原 45 分钟
总上限，被 GitHub 取消。此预算覆盖安装、完整门禁和收尾，不改变单项测试时限、断言或覆盖率阈值。
...
GitHub macOS CI 的 `query-interaction.spec.ts` 两项 DOM 耗时测试使用软件渲染。原生跟踪确认
Apple Paravirtual GPU 存在没有对应主线程长任务的帧等待；软件后端在有、无剖析的各 40 项
原断言中通过，未增加重试。仍保留 Popup <200 ms、缓存显示 <100 ms、流式显示 <250 ms，
以及中文内容、焦点、滚动、主题和模型调用次数断言。其他平台、本机 Mac、实际扩展加载、
视频解码和官网测试配置不受影响；此夹具不代表 Mac 物理 GPU 性能验证。准确对照见
[合并前全面检查回执](cloud-v1/asbplayer-final-review.md)。临时剖析钩子与专项工作流已移除。
