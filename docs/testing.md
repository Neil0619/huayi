...
- YouTube 字幕：标准 `/watch` 判断、可见片段视觉排序、英文与长度过滤、撇号/连字符分词、
  跨 cue 重叠去重、自动字幕修订、句界与 2.5 秒退化、2,000 字符有界缓存、补全期禁用取词、
  精确目标句上下文、鼠标位置锚定、智能暂停恢复、“译”按钮切换关闭、连续取词、SPA/seek
  清理和全屏挂载。
...
- 本地 YouTube DOM fixture 覆盖 CC 旁取词按钮、跨 cue 实时补句、冻结字幕、精确点词、拖选
  短语、整句字幕按钮与鼠标位置附近的操作浮层、“译”按钮二次点击关闭、最终请求只含目标句，
  以及 warmup 不携带字幕数据。
