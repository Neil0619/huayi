...
“语见”是一款 Chrome 英文划词翻译与用法分析产品。双击或拖选英文后，可查看词典式翻译、
语境解释和流式结构化结果；观看普通 YouTube 录播视频时，也可直接选择语见英文字幕中的
单词或短语，并在英文与快速双语字幕之间切换。

当前版本为 `0.13.0`，Native Messaging 协议为 `schemaVersion: 7`。扩展提供统一配置页和快捷
弹窗，可配置总开关、默认动作、网站黑白名单、Provider、生词本同步以及 YouTube 字幕偏好。
欧路收藏可按日进入扇贝；扇贝拒绝的词会在本机离线尝试唯一词元，无法可靠处理的词保留在
人工列表中。
...
在普通 YouTube `/watch` 录播中先由用户开启英文字幕。语见验证当前活动英文轨后，以可原生
选择的完整英文句替代 YouTube 字幕；双击选择单词、拖选短语后仅在视频原本播放时暂停，并
继续使用既有“解释｜翻译”和欧路生词本；拖选完整句子时按句子请求翻译或解释。关闭结果或
点击播放器视频空白区时，只恢复由语见暂停的视频。

每个新视频默认英文或双语，可在配置页设置。译轨可用时，点击 CC 旁“中”固定双语，或按住
已配置快捷键（默认 `Shift+Z`）／按住字幕角标临时查看中文；中文来自当前英文轨的 YouTube
`zh-Hans` 自动翻译，只保留在当前页面内存，不调用模型或 Host。
语见不替用户开启 CC 或切换字幕轨；源轨失败、关 CC、切非英文轨、广告、直播或导航离开时
