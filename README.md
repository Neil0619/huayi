...
Responses API、官方 DeepSeek Chat Completions，或在明确接受明文 HTTP 风险后切换到独立配置
的 OpenAI-compatible Provider。三种 API Key 分别保存在独立的 macOS 钥匙串项中，不进入扩展。
...
- 单词翻译现在是词典查询：顶部音标、语境义、按词性分组的现代常见义、常用短语和公认
  易混词；不再重复显示原文例句或独立词性板块。
- 单词解释现在是用法分析：语境取义原因、词形与句法作用、可靠构词、用法要点和带差异说明
  的语境同义词。易混词与同义词使用独立协议类型，不得混用。
- 短语、单句和段落行为保持不变，所有最终结果继续经过严格结构化校验。
...
- 单词分析期间并行查询欧路生词本；已存在的单词在加载或结果阶段显示紧凑的“已加入”状态。
...
- 可选 DeepSeek Provider 固定调用 `https://api.deepseek.com/chat/completions`，使用
  `deepseek-v4-flash`、非思考模式、JSON Output、`temperature: 0` 和严格 data-only SSE；它
  每次从独立钥匙串读取 Key，不读取或修改 `~/.codex`，失败时也不自动回退其他 Provider。
- 流式更新按 `requestAnimationFrame` 每帧最多渲染一次，并按 `data-huayi-section` 复用稳定
  DOM 节点；数组项逐条追加，已有节点不会因最终校正而整卡替换。
- 浮层更新为轻量词典卡：白色单层表面、源词与音标头部、唯一的语境强调区、无盒化内容行、
  紧凑右上角生词动作和 320px 单列布局；加载及单词流式阶段保留稳定最小高度以减少跳动。
...
v0.9.0 继续使用 `schemaVersion: 5` 并拒绝 v4。Extension 与 Native Host 必须同步升级或回滚；
...
DeepSeek 也必须按“配置、真实 smoke、切换”三个独立动作启用；配置 Key 不改变当前 Provider：
...
真实 smoke 只发送固定英文用例并输出匿名首个可见内容与完整结果耗时，不读取 Codex 配置，
也不会切换 Provider。回滚后续请求仍只需 `pnpm host:provider:set codex`。
...
v0.9.0 只支持 macOS、Google Chrome 和普通 `http/https` 顶层网页，不支持 PDF、Chrome
...
欧路或钥匙串。`pnpm smoke:codex`、`pnpm smoke:compatible`、`pnpm smoke:compare` 和
`pnpm smoke:deepseek` 会调用真实模型并消耗订阅额度或 API 费用，只能在用户明确批准后单独
执行；真实安装、Keychain、
