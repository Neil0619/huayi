...
BEGIN
  ...
  IF flow.kind='login' THEN
    IF sign_in_method <> 'google' THEN RETURN NULL; END IF;
  ...
  END IF;
  IF flow.kind <> 'invite-registration' THEN RETURN NULL; END IF;
...
REVOKE ALL ON FUNCTION complete_auth_flow(text,uuid,text,text,integer,text)
FROM PUBLIC,huayi_business,huayi_runtime;
GRANT EXECUTE ON FUNCTION complete_auth_flow(text,uuid,text,text,integer,text)
TO huayi_context_setter;

CREATE FUNCTION refresh_profile_email(
  account_user_id uuid,
  account_email text
) RETURNS boolean
LANGUAGE sql
...
AS $$
  WITH refreshed AS (
    ...
    SET email=account_email,updated_at=now()
...
REVOKE ALL ON FUNCTION refresh_profile_email(uuid,text)
FROM PUBLIC,huayi_business,huayi_runtime;
GRANT EXECUTE ON FUNCTION refresh_profile_email(uuid,text)
TO huayi_context_setter;

DO $repair_password_callback_method$
BEGIN
  IF to_regclass('auth.identities') IS NOT NULL THEN
    EXECUTE $repair$
      UPDATE public.account_sign_in_methods AS methods
      SET method='password'
      WHERE methods.method='google'
        AND EXISTS (
          SELECT 1 FROM public.invitation_claims AS claims
          WHERE claims.finalized_user_id=methods.owner_user_id
        )
        AND EXISTS (
          SELECT 1 FROM auth.identities AS identities
          WHERE identities.user_id=methods.owner_user_id AND identities.provider='email'
        )
        AND NOT EXISTS (
          SELECT 1 FROM auth.identities AS identities
          WHERE identities.user_id=methods.owner_user_id AND identities.provider='google'
        )
        AND NOT EXISTS (
          SELECT 1 FROM public.account_sign_in_methods AS existing
          WHERE existing.owner_user_id=methods.owner_user_id AND existing.method='password'
        )
    $repair$;
  END IF;
END;
$repair_password_callback_method$;
