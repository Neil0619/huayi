...
CREATE TABLE invitations (
  ...
  CONSTRAINT invitations_created_by_kind_check CHECK (
...
CREATE TABLE security_notification_outbox (
  ...
  CONSTRAINT security_notification_outbox_delivery_deadline_check
...
CREATE TABLE huayi_private.first_operator_bootstrap (
  singleton boolean PRIMARY KEY DEFAULT true CHECK (singleton),
  state text NOT NULL CHECK (state IN ('invited', 'completed')),
  current_invitation_id uuid NOT NULL REFERENCES invitations(id),
...
CREATE FUNCTION huayi_private.clear_deleted_first_operator_identity()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, huayi_private
...
CREATE TABLE model_rate_limit_events (
  owner_user_id uuid NOT NULL REFERENCES user_profiles(user_id) ON DELETE CASCADE,
  request_id uuid PRIMARY KEY,
  occurred_at timestamptz NOT NULL,
  CHECK (request_id IS NOT NULL)
);
CREATE INDEX model_rate_limit_events_owner_occurred
...
CREATE FUNCTION huayi_private.issue_first_operator_invitation(
...
CREATE FUNCTION huayi_private.replace_first_operator_invitation(
  new_invitation_id uuid,
  new_token_hash text,
  new_expires_at timestamptz,
  operation_time timestamptz
...
CREATE FUNCTION huayi_private.complete_first_operator_bootstrap(operation_time timestamptz)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, huayi_private
...
CREATE FUNCTION ensure_current_default_quota(
  account_user_id uuid,
  operation_time timestamptz
) RETURNS uuid
LANGUAGE plpgsql
...
CREATE FUNCTION ensure_owner_current_default_quota(
  account_user_id uuid,
  operation_time timestamptz
) RETURNS uuid
LANGUAGE plpgsql
...
CREATE FUNCTION analysis_reservation_amount(
  account_user_id uuid, request_id uuid, lease_token text, reservation_id uuid
...
CREATE FUNCTION huayi_private.owner_analysis_public_record(
  account_owner_user_id uuid, target_record_id uuid
) RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog AS $$
  SELECT huayi_private.analysis_public_record(target_record_id)
  FROM public.analysis_records records
  WHERE records.id=target_record_id
    AND records.owner_user_id=account_owner_user_id
    AND account_owner_user_id=huayi_private.current_owner_user_id();
$$;
...
CREATE FUNCTION replay_account_deletion(
...
BEGIN
  ...
  INSERT INTO public.security_notification_outbox(
    ...
    notification_id,current_flow.owner_user_id,'password-reset-completed',completed_at,completed_at,
    completed_at+interval '23 hours'
  );
  RETURN true;
END;
$$;
...
REVOKE ALL ON FUNCTION analysis_reservation_amount(uuid, uuid, text, uuid)
  FROM PUBLIC, huayi_business;
REVOKE ALL ON FUNCTION huayi_private.analysis_public_record(uuid) FROM PUBLIC, huayi_business;
REVOKE ALL ON FUNCTION huayi_private.owner_analysis_public_record(uuid, uuid)
  FROM PUBLIC, huayi_business;
...
GRANT EXECUTE ON FUNCTION analysis_reservation_amount(uuid, uuid, text, uuid)
  TO huayi_context_setter;
GRANT EXECUTE ON FUNCTION huayi_private.owner_analysis_public_record(uuid, uuid)
  TO huayi_context_setter;
...
CREATE POLICY model_rate_limit_events_owner ON model_rate_limit_events USING (owner_user_id = huayi_private.current_owner_user_id()) WITH CHECK (owner_user_id = huayi_private.current_owner_user_id());
