...
export const phase93FreshCsrfVercelDiagnosticArgument =
  "--diagnose-hosted-vercel-phase-93-0023-fresh-csrf-neil0619s-projects";
...
export const phase93FreshCsrfVercelDiagnosticFieldNames = fields;
...
export async function runPhase93FreshCsrfVercelDiagnosticCli({
  ...
  if (arguments_.length !== 1 || arguments_[0] !== phase93FreshCsrfVercelDiagnosticArgument) {
    writeError("Hosted Phase 93 fresh-CSRF Vercel one-shot diagnostic failed.\n");
    return 1;
  }
  const completionStore =
    historicalStateStore ??
    createVercelOneShotStateStore({ repositoryRoot, stateIdentity: "phase-93-0023" });
  const currentStore =
    stateStore ??
  ...
  let historicalStateComplete = false;
  try {
    const historicalState = await completionStore.read();
    historicalStateReadable = true;
    validatePhase93VercelCompletion(historicalState);
    historicalStateComplete = true;
  } catch {
    historicalStateComplete = false;
  }
  ...
  const api = projectDiagnostic(snapshot?.api, phase93FreshCsrfVercelOneShotBaselines.api);
  const web = projectDiagnostic(snapshot?.web, phase93FreshCsrfVercelOneShotBaselines.web);
  ...
  const candidateBaselineExact =
    snapshot !== undefined &&
    api.nonCanceledCount === phase93FreshCsrfVercelOneShotBaselines.api.count &&
    api.inFlightCount === 0 &&
    api.latestState === "READY" &&
    api.latestIdCandidate &&
    api.latestCommitCandidate &&
    web.nonCanceledCount === phase93FreshCsrfVercelOneShotBaselines.web.count &&
    web.inFlightCount === 0 &&
  ...
  const values = {
    ...
    historical_state_complete: truth(historicalStateComplete),
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.exitCode = await runPhase93FreshCsrfVercelDiagnosticCli();
}
