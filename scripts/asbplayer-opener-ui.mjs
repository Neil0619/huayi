...
<main><p class="muted">语见 · 本机视频</p><h1>从原视频开始学习</h1><p>选择 MKV 或 MP4。自动准备浏览器音轨和文字字幕，原视频保持不变。</p><button id="open">选择原视频</button><button id="learn" disabled>开始学习</button><p id="status" role="status">准备就绪</p><h2 id="title"></h2><div id="subtitles"></div><p id="notice" class="muted"></p><p class="muted">再次点击“选择原视频”即可换片。处理结果仅保存在本机缓存；第一次准备需要一些时间，再次打开会复用缓存。</p><p class="muted">请在已加载语见扩展的 Chrome 中使用。图像字幕暂不支持学习，可另选文字字幕。</p><button id="subtitle" disabled>添加文字字幕</button><button id="close">退出打开器</button></main><script src="/opener.js"></script></html>`;
...
export function openerClient(view, doc) {
  ...
  const refresh = async () => {
    try {
      ...
      open.disabled = current.status === "busy" || importing;
      doc.querySelector("#close").disabled = current.status === "busy" || importing;
      doc.querySelector("#subtitle").disabled = current.status !== "ready" || importing;
      learn.disabled = current.status !== "ready" || importing;
  ...
  learn.onclick = () => {
    ...
    const receive = async (event) => {
      ...
      try {
        ...
        cleanup();
        status.textContent = "文件导入失败，请重新打开原视频后再试。";
      }
    };
    const cleanup = () => {
      active = false;
