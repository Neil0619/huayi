...
import {
  hostedMigration0022StatusPredicateNames as predicateNames,
  renderHostedMigration0022StateCtes,
} from "./acceptance-hosted-migration-0022-status-contract.mjs";
import { fetchHostedAcceptanceOfficialCaCertificate } from "./acceptance-hosted-official-ca.mjs";

export const hostedMigration0022StatusDiagnosticPredicateNames = predicateNames;
export const hostedMigration0022StatusDiagnosticArgument = `--diagnose-status-20260828010000-password-signup-expired-invitation-recovery-${hostedAcceptanceProjectRef}`;
...
function renderSetupFailure(stage) {
  if (!setupFailureStages.has(stage)) {
    throw new Error("Hosted 0022 status diagnostic stage is invalid.");
  }
  return `Hosted 0022 status diagnostic failed at allowlisted stage ${stage}.`;
}

export function renderHostedMigration0022StatusDiagnosticSql() {
...
SELECT diagnostic.name || '|' || diagnostic.value AS diagnostic
FROM status_state
...
export function classifyHostedMigration0022StatusDiagnosticExitCode(code) {
  if (code === 0) return "ok";
...
export function parseHostedMigration0022StatusDiagnosticOutput(output) {
...
export async function runHostedMigration0022StatusDiagnosticQuery(
  ...
  const result = await runPsql({
    ...
    input: renderHostedMigration0022StatusDiagnosticSql(),
    timeoutMilliseconds: 30_000,
  });
  const exitClass = classifyHostedMigration0022StatusDiagnosticExitCode(result.code);
  const diagnostic =
    result.code === 0 ? parseHostedMigration0022StatusDiagnosticOutput(result.stdout) : null;
...
function renderDiagnosticResult({ diagnostic, exitClass, outputExact }) {
  ...
  const safeDiagnostic = safeOutputExact
    ? diagnostic
    : { finalStatus: "uncertain", predicates: falsePredicates() };
  return [
...
export async function runHostedMigration0022StatusDiagnosticCli({
  arguments_ = process.argv.slice(2),
  environment = process.env,
  fetchCaCertificate = fetchHostedAcceptanceOfficialCaCertificate,
  readPassword = readHiddenTerminalLine,
  runDiagnosticQuery = runHostedMigration0022StatusDiagnosticQuery,
  ...
  try {
    if (
      arguments_.length !== 1 ||
      arguments_[0] !== hostedMigration0022StatusDiagnosticArgument ||
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.exitCode = await runHostedMigration0022StatusDiagnosticCli();
}
