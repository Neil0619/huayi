...
const failureMessage = "Hosted 0015 migration dry-run failed closed; database was not modified.";
const realCertificateIo = Object.freeze({ chmod, mkdtemp, rm, writeFile });

export const hostedMigration0015Filename = "20260825010000_public_function_acl_hardening.sql";
export const hostedMigration0015DryRunArgument = `--confirm-dry-run-20260825010000-public-function-acl-hardening-${hostedAcceptanceProjectRef}`;
export const hostedMigration0015SuccessMessage = `Supabase migration dry-run passed: exactly ${hostedMigration0015Filename}; database was not modified.`;
...
function classifyDryRunChannel(output) {
  ...
  for (const line of lines) {
    const index = /^(?: )?• 20260825010000_public_function_acl_hardening\.sql$/u.test(line)
      ? 3
...
export function classifyHostedMigration0015DryRunTranscript({ stderr, stdout } = {}) {
...
export function hasExactHostedMigration0015DryRunTranscript({ stderr, stdout } = {}) {
  return classifyHostedMigration0015DryRunTranscript({ stderr, stdout }).transcriptExact;
}

export function parseHostedMigration0015DryRunOutput(output) {
  return classifyHostedMigration0015DryRunTranscript({ stderr: output, stdout: "" })
    .transcriptExact;
}

export async function runHostedMigration0015DryRunProcess(
...
export async function runHostedMigration0015DryRunCli({
  arguments_ = process.argv.slice(2),
  environment = process.env,
  fetchCaCertificate = fetchHostedAcceptanceOfficialCaCertificate,
  readPassword = readHostedAdministratorPassword,
  runSupabase = runHostedMigration0015DryRunProcess,
  ...
  try {
    if (
      arguments_.length !== 1 ||
      arguments_[0] !== hostedMigration0015DryRunArgument ||
    ...
    const result = await runSupabase({ administratorPassword, caCertificate });
    if (result.code !== 0 || !hasExactHostedMigration0015DryRunTranscript(result)) {
      throw new Error(failureMessage);
    }
    writeOutput(`${hostedMigration0015SuccessMessage}\n`);
    return 0;
  } catch {
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.exitCode = await runHostedMigration0015DryRunCli();
}
