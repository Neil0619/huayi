...
describe("RequestCoordinator warmup", () => {
  it("deduplicates pending and ready warmup, then resets after disconnect", () => {
    ...
    coordinator.warmup();
    coordinator.warmup();
    expect(transport.sent).toEqual([{ requestId: "control-1", schemaVersion: 7, type: "warmup" }]);
  ...
  it("resets failed warmup without delivering its error to a tab", () => {
    ...
    expect(transport.sent).toEqual([
      { requestId: "control-1", schemaVersion: 7, type: "warmup" },
      { requestId: "control-2", schemaVersion: 7, type: "warmup" },
    ]);
    expect(delivered).toEqual([]);
    coordinator.dispose();
  });
