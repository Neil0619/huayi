...
test("keeps an above-selection result anchored when its explanation expands and collapses", async ({
  page,
}) => {
  await page.setViewportSize({ width: 900, height: 900 });
  await page.goto("/apps/store-extension/e2e/fixtures/query-interaction.html?explicit-structure");
  await expect(page.locator("body")).toHaveAttribute("data-ready", "true");
  const original = page.locator("#original");
  await original.evaluate((element) => {
    element.textContent = "We agree on the plan.";
    element.setAttribute("style", "position:absolute;left:100px;top:820px;margin:0");
  });
  ...
  if (!closed) throw new Error("Missing result bounds");
  await panel.locator("summary").click();
  await expect(panel.locator("details")).toHaveAttribute("open");
  expect((await panel.boundingBox())?.height).toBeGreaterThan(closed.height);
  await expect
    .poll(async () => {
      const bounds = await panel.boundingBox();
      return bounds ? Math.abs(bounds.y + bounds.height - closed.y - closed.height) : Infinity;
    })
    .toBeLessThanOrEqual(1);
  await expect(panel.locator(".footer")).toBeInViewport({ ratio: 1 });
  await panel.locator("summary").click();
  await expect.poll(async () => (await panel.boundingBox())?.y).toBeCloseTo(closed.y, 0);
});
