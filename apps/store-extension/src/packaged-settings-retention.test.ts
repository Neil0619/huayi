...
// Build in setup so three runtime restarts still share the unchanged default test deadline.
let source: string;
let directory: string | undefined;
beforeAll(async () => {
  directory = await mkdtemp(join(tmpdir(), "huayi-settings-retention-"));
...
it("restarting the packaged hosted worker preserves settings, encrypted credentials and pairing without session storage", async () => {
  ...
  for (let restart = 0; restart < 3; restart++) {
    ...
    const worker = loadPackagedWorker(source, "hoijjhgcckfhbcefoclgbhkgninnkknd", restartedStorage);
