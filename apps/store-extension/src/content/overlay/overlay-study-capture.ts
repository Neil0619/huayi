...
export class OverlayStudyCapture {
  ...
  private state: StoreStudyCaptureResponse | null = null;

  constructor(private readonly options: OverlayStudyCaptureOptions) {}
  ...
  render(body: HTMLElement, selection: StoreSelectionReading): void {
    ...
    if (outcome === "created" || outcome === "queued") {
      status.textContent = outcome === "created" ? "已加入待学习" : "待联网加入";
      ...
      status.textContent = "登录后可加入待学习";
      action.hidden = true;
    } else {
      status.textContent =
        outcome === "failed" ? "加入失败，可重试" : "保存原句，稍后在 Web 深度分析";
      action.textContent = outcome === "failed" ? "重试加入" : "加入待学习";
  ...
  private async create(trigger: "automatic" | "manual"): Promise<void> {
