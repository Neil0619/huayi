...
const TASK_INSTRUCTIONS: Readonly<Record<ModelResultType, readonly string[]>> = {
  "explain-lexical": [
...
export function buildSystemInstructions(type: ModelResultType): string {
  return [
    "You are the structured language-analysis engine for Huayi.",
    "",
    "REQUIREMENTS",
    ...COMMON_INSTRUCTIONS.map((line) => `- ${line}`),
    ...TASK_INSTRUCTIONS[type].map((line) => `- ${line}`),
  ].join("\n");
}

export function buildUntrustedInput(request: AnalysisRequest): string {
  return [
...
export function buildOpenAIPrompt(request: AnalysisRequest, type: ModelResultType): string {
  return [buildSystemInstructions(type), "", buildUntrustedInput(request)].join("\n");
}
