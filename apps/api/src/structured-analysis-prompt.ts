...
export function structuredAnalysisInstructions(
  ...
  return [
    ...
    "Choose short reusable literal expressions, not whole news facts with specific people, places or incidental details. If a long quote cannot appear unchanged in a natural new example, choose a shorter literal chunk or verb from the source; explain the wider construction in usageZh. An expression such as prevent must appear as prevent, not prevents or prevented, in its example. Do not select an expression until you can write a natural new sentence containing its exact unchanged text.",
    kind === "phrase"
    ...
    "For a pattern example, choose exampleValues first, literally substitute them into template, then set generatedExample.sourceText to exactly that rendered string. Do not add a subject, prefix, suffix or final punctuation outside the template, and do not inflect its fixed words. If this does not form a natural complete sentence, redesign the complete source-backed clause template and both sets of values, or select a shorter literal expression instead.",
    ...(kind === "phrase"
      ? []
      : [
