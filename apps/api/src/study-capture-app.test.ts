...
describe("StudyCapture HTTP adapter", () => {
  ...
  it.each(["X-Huayi-Revision", "If-Match"])(
    "provides revision-proven patch routes using %s",
    async (revisionHeader) => {
      const capture = {
        captureCount: 1,
        createdAt: "2026-08-13T00:00:00.000Z",
        firstCapturedAt: "2026-08-13T00:00:00.000Z",
      ...
      expect((await app.request("/v1/study-captures?status=pending&kind=sentence")).status).toBe(
        200,
      );
