...
export function buildDeepSeekAnalysisRequest(
  ...
  if (repairContent !== undefined) {
    ...
    const instruction = (content: string) =>
      [
        "outputContract" in input
          ? "Repair the rejected structure and its dependent source/example witnesses together. Preserve the intended teaching and return a JSON object that strictly follows the required shape."
        ...
        "For recommendation-example-expression, generatedExample.sourceText must contain the full literal candidate expression with word boundaries, preserving its spelling, case and punctuation. Never paraphrase or inflect the source expression to force a match. If it is a long news fact that cannot transfer naturally, select a shorter exact source chunk and update candidate text, usage, sourceRefs and the example together; otherwise repair the example to contain the unchanged expression.",
        "For recommendation-source-expression, learningAdvice.sourceRefs must reference the full literal candidate expression in its own source unit at word boundaries. Use one exact {text,occurrence} reference. Check that candidate.text itself is copied from this unit, not lemmatized, paraphrased or embedded inside another word; if necessary restore its exact source form and update the example consistently. Never invent source text or drop learningAdvice to evade validation.",
        input.selectionKind === "phrase"
          ? "Phrase candidates must remain expressions: repair any gap notation to a literal expression from the supplied phrase and make the example contain that full unchanged text. Never convert a phrase candidate to sentence_pattern."
          : "If an expression uses gaps such as ... or sb/sth, represent the intended reusable construction as a source-backed sentence_pattern with declared {name} placeholders, sourceValues and exampleValues; do not replace it with a long literal source quote while keeping an incompatible example. For slot-used-in-template or template-slot-reference, use curly-braced {name} placeholders, not bare variable names. Recheck all candidate source refs and examples after any candidate change, including punctuation: a final period belongs consistently in template, full sourceRef and generatedExample.",
        "For recommendation-source-pattern, at least one sourceRef must contain the entire template reconstructed with sourceValues, including its final punctuation when present in the original unit. Never invent text or borrow it from another unit.",
        "For recommendation-example-template, choose exampleValues first, literally substitute every declared slot into template, and set generatedExample.sourceText to exactly that rendered string. Do not prepend a subject, add a prefix/suffix/punctuation or inflect fixed words: prevent and prevents are different. If the result is not a natural complete sentence, redesign a complete source-backed clause template and all source/example values together, or choose a shorter unchanged source expression with a matching new example. Do not drop learningAdvice to evade either recommendation rule.",
        "VALIDATION_FAILURES",
