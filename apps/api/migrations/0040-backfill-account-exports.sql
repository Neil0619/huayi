...
ALTER TABLE public.account_data_export_jobs
  ADD CONSTRAINT account_data_export_jobs_format_version_check CHECK (format_version IN (1,2,3,4,5));

-- Existing claim functions remain frozen so older workers cannot claim format 5.
CREATE FUNCTION public.claim_account_export_v5(
...
BEGIN
  ...
  WHERE jobs.format_version IN (1,2,3,4,5) AND profiles.status IN ('active','disabled') AND
...
REVOKE ALL ON FUNCTION public.claim_account_export_v5(text,timestamptz)
  FROM PUBLIC,anon,authenticated,service_role,huayi_business;
GRANT EXECUTE ON FUNCTION public.claim_account_export_v5(text,timestamptz)
