...
ALTER TABLE public.account_data_export_jobs
  ADD CONSTRAINT account_data_export_jobs_format_version_check CHECK (format_version IN (1,2,3,4));

-- Existing v1/v2/v3 claim functions remain frozen so older workers cannot claim format 4.
CREATE FUNCTION public.claim_account_export_v4(
...
BEGIN
  ...
  WHERE jobs.format_version IN (1,2,3,4) AND profiles.status IN ('active','disabled') AND
...
REVOKE ALL ON FUNCTION public.claim_account_export_v4(text,timestamptz)
  FROM PUBLIC,anon,authenticated,service_role,huayi_business;
GRANT EXECUTE ON FUNCTION public.claim_account_export_v4(text,timestamptz)
