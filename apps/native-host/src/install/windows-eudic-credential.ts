...
const CONFIGURE_ACTION = "Configure Windows DPAPI-protected Eudic credential";
const REMOVE_ACTION = "Remove Windows DPAPI-protected Eudic credential";
...
export interface ConfigureWindowsEudicOptions extends BaseOptions {
  readonly interactiveProcessRunner: InteractiveProcessRunner;
}

export interface RemoveWindowsEudicOptions extends BaseOptions {
...
async function validateHelpers(options: BaseOptions): Promise<void> {
  try {
    ...
    throw new Error("Windows PowerShell or the Huayi Eudic credential helper is unavailable.");
  }
}
...
export async function configureWindowsEudicCredential(
  options: ConfigureWindowsEudicOptions,
  ...
  if (result.exitCode !== 0 || result.signal !== null) {
    throw new Error("Unable to configure the Windows Eudic credential.");
  }
  return { actions: [CONFIGURE_ACTION], dryRun: false };
}

export async function removeWindowsEudicCredential(
  options: RemoveWindowsEudicOptions,
): Promise<CredentialOperationResult> {
  await validateHelpers(options);
  ...
  if (result.exitCode !== 0 || result.signal !== null) {
    throw new Error("Unable to remove the Windows Eudic credential.");
  }
