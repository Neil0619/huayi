...
import {
  ...
  pendingSentence,
  dialogueStart,
} from "./cloud-browser-practice-seed.js";
...
export function createCloudBrowserPracticeAuthority(seed: PracticeAuthoritySeed) {
  ...
  const queue = () => {
    const currentSession = session?.items.some((item) => item.rating === undefined)
      ? session
      : null;
      ...
      items:
        seed === "empty-practice"
          ...
          : queueItems.filter(
              (entry) =>
                !session?.items.some(
                  (item) => item.itemId === entry.item.id && item.rating !== undefined,
                ),
            ),
  ...
  const handle = async (route: Route, hooks: Hooks): Promise<boolean> => {
    ...
    if (
      session &&
      path === `/v2/practice/sessions/${session.id}/teaching` &&
      request.method() === "GET"
    ) {
