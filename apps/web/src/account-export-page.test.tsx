...
it("requests format 5 explicitly and preserves format 3 for an existing saved job", async () => {
  const { node, api } = await mount();
  await act(async () => node.querySelector<HTMLButtonElement>("[data-create-export]")?.click());
  expect(api.createAccountDataExport).toHaveBeenCalledWith(5);
...
it("releases an old busy action when a new API scope loads and ignores the old completion", async () => {
  ...
  await act(async () => node.querySelector<HTMLButtonElement>("[data-create-export]")?.click());
  expect(next.createAccountDataExport).toHaveBeenCalledWith(5);
});
