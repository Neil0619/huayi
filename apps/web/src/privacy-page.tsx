        ...
        <article className="privacy-notice">
          ...
          <section aria-labelledby="data-title">
            ...
            <ul>
              ...
              <li>
                连接安全：为登录、注册与设备配对等接口防止滥用，服务端处理连接
                IP，保存结合服务器秘密计算的限流标识、操作类型、时间窗口和计数；不使用
                GPS，也不据此生成广告画像。
              </li>
              <li>
                扇贝回填：主动开启共享后的来源词头、目标词、设置和成功、跳过或结果待确认进度；不上传欧路或本机词的解释、原句、URL
                或标题。
              </li>
              <li>账号数据导出：用户显式请求时生成的私有、限时完整账号导出对象。</li>
              <li>
                错误诊断：语见服务自动记录错误码、阶段、状态码、耗时、版本、账号及请求关联编号。
                插件仅在你独立开启“自动上传错误诊断”并连接账号后自动上报；不包含原文、URL、模型回答、密钥或原始异常文本。
              </li>
            </ul>
            <p>
              插件在本机处理当前网址、域名及 YouTube 视频标识，用于逐站规则和字幕等页面功能，不读取
              Chrome
              浏览历史列表，也不自动上传完整网页或完整浏览历史。模型请求只包含所选内容及必要语境；学习内容的来源标题由用户填写或受信的固定来源类型提供。
            </p>
          </section>
          ...
          <section aria-labelledby="recipients-title">
            ...
            <ul>
              ...
              <li>
                欧路在用户同意后用于手动导入、导出和主动开启的每日回填检查。扇贝每批最多预填 100
                个目标词，最终添加始终由用户亲自点击。
              </li>
              <li>
                开启账号共享后，语见按账号保存回填账本并每 15
                分钟检查云端词头。欧路与本机词头不会因此成为云端学习库词条。停用保留进度，删除学习词条不清除成功与跳过历史；完整账号导出包含共享回填记录，删除账号清除账本。第三方已收到的数据须在对应服务中删除。
              </li>
              <li>
                首次检查包含已同意来源的已有词；欧路默认在本地时间 08:00
                检查，浏览器恢复后补查。本机生词、欧路和云端生词按词头去重，不调用模型。未关联账号时进度只保存在本机；共享进度的设备应使用同一扇贝账号，语见不读取扇贝完整词表，也不能验证扇贝账号身份。
              </li>
            </ul>
            <p>
          ...
          <section aria-labelledby="retention-title">
            ...
            <ul>
              ...
              <li>
                错误诊断仅供管理员排查，保留 30 天。插件诊断暂存最多 100 条、256 KiB、24
                小时；关闭诊断或退出账号会清除待上传日志。
              </li>
              <li>
                连接安全的限流记录尚未设置自动删除期限；计数窗口不代表记录保留期限。这类记录使用哈希标识，删除账号不保证同步清除。托管服务的访问日志按其服务设置与隐私规则处理，不适用上述错误诊断的
                30 天期限。
              </li>
            </ul>
            {production && (
